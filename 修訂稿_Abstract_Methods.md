...
## 7. 9TPG 上多方法完整對照表(DeepAllo / AlloFusion / PASSer 2.0 / AllositePro / STINGAllo)

### 7.1 方法可測試性總結

| 方法 | 9TPG 可測試性 | 實際測試結果 | 備註 |
|---|---|---|---|
| **DeepAllo (官方)** | ✅ 本機完整重現 | 16 口袋概率 0.013–0.018，**無別構預測** | 1MC0 陽性對照 11/11 命中驗證管線 |
| **AlloFusion** | ✅ 本機重現(ProtT5-XL + CNN) | 181 殘基概率 0.0002–0.118，**無別構預測**(最高 0.118 ≪ 0.5) | PSSM/bio features 以零向量代替；1MC0 未驗證 |
| **PASSer 2.0** | ❌ 無法測試 | Server 回「not in PDB」 | PDB 資料庫截止 2024-11，9TPG (2026) 未收錄 |
| **AllositePro** | ❌ 無法測試 | Web server (mdl.shsmu.edu.cn/AST/) 無 API，需人工上傳 | 作者未提供 CLI/批次介面 |
| **STINGAllo** | ❌ 無法測試 | API 回 404 | 227,316 結構資料庫截至 2024-11 |

### 7.2 逐殘基概率對照 (AlloFusion vs DeepAllo vs Ground Truth)

| Residue (pdb_seq_num) | In GT (A1H2 contact) | DeepAllo pocket prob | AlloFusion prob | Notes |
|---|---:|---:|---:|---|
| 1089 | ✓ | — (bg) | 0.0097 | GT only |
| 1095 | ✓ | — (bg) | 0.0074 | GT only |
| 1113 | ✓ | — (bg) | 0.0043 | GT only |
| **1116** | ✓ | **0.0183 (P1)** | **0.0129** | **P1∩GT** |
| 1117 | ✓ | 0.0183 (P1) | 0.0075 | P1∩GT |
| 1118 | ✓ | 0.0183 (P1) | 0.0063 | P1∩GT |
| 1123 | ✓ | — (bg) | 0.0075 | GT only |
| 1124 | ✓ | — (bg) | 0.0101 | GT only |
| **1125** | ✓ | **0.0183 (P1)** | **0.0064** | **P1∩GT** |
| **1126** | ✓ | **0.0183 (P1)** | **0.0086** | **P1∩GT** |
| **1127** | ✓ | **0.0183 (P1)** | **0.0061** | **P1∩GT** |
| **1128** | ✓ | **0.0183 (P1)** | **0.0069** | **P1∩GT** |
| **1129** | ✓ | **0.0183 (P1)** | **0.0021** | **P1∩GT** |
| 1139 | ✓ | — (bg) | 0.0041 | GT only |
| 1147 | ✓ | — (bg) | 0.0091 | GT only |
| 1148 | ✓ | — (bg) | 0.0225 | GT only |
| 1149 | ✓ | — (bg) | 0.0098 | GT only |
| 1150 | ✓ | — (bg) | 0.0128 | GT only |
| 1151 | ✓ | — (bg) | 0.0163 | GT only |
| 1152 | ✓ | — (bg) | 0.0079 | GT only |
| 1153 | ✓ | — (bg) | 0.0151 | GT only |
| 1154 | ✓ | — (bg) | 0.0116 | GT only |
| **AlloFusion top-5 (non-GT)** | — | — | 0.118 / 0.028 / 0.026 / 0.024 / 0.024 | resSeq 229, 118, 93, 90, 139 |
| **DeepAllo P1 max** | 6/22 GT | **0.0183** | — | Pocket 1: 1116, 1125–1129 |
| **DeepAllo P2** | 0/22 GT | 0.0167 | — | Pocket 2: 1157, 1159–1161 |
| **DeepAllo P3** | 0/22 GT | 0.0149 | — | Pocket 3: 56,57,58,60,62, 1098,1105–1108 |

### 7.3 整體指標摘要 (9TPG, GT = 22 A1H2-contact residues)

| 方法 | 評估模式 | n_pred | TP | FP | FN | Precision | Recall | F1 | Site Hit | Max Prob |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| **DeepAllo** | Top-1 pocket | 6 | 6 | 0 | 16 | 1.000 | 0.273 | 0.429 | 1 | 0.0183 |
| **DeepAllo** | Top-3 pockets | 20 | 6 | 14 | 16 | 0.300 | 0.273 | 0.286 | 1 | 0.0183 |
| **DeepAllo** | Full (16 pockets) | 20 | 6 | 14 | 16 | 0.300 | 0.273 | 0.286 | 1 | 0.0183 |
| **AlloFusion** | prob > 0.5 | 0 | 0 | 0 | 22 | — | 0.000 | — | 0 | 0.118 |
| **AlloFusion** | Top-1 residue | 1 | 0 | 1 | 22 | 0.000 | 0.000 | 0.000 | 0 | 0.118 |
| **AlloFusion** | Top-6 residues* | 6 | 0 | 6 | 22 | 0.000 | 0.000 | 0.000 | 0 | 0.118 |

*AlloFusion top-6 residues (resSeq 229, 118, 93, 90, 139, 42) — none in GT (22 residues: 1089–1154)

### 7.4 結論性陳述

1. **DeepAllo (官方)** 在 9TPG 上給出**均一背景級別概率 (0.013–0.018)**，無口袋達決策閾值 0.5。1MC0 陽性對照同管線給出 0.844 概率、11/11 完全命中 → 管線正確，9TPG 為真實陰性。

2. **AlloFusion** 本機重現得出**最高概率 0.118 (resSeq 229)**，遠低於 0.5 閾值；Top-6 預測殘基(229, 118, 93, 90, 139, 42)皆不在 GT (1089–1154) 中 → **同樣為陰性結果**。

3. **PASSer 2.0 / AllositePro / STINGAllo** 均**無法在 9TPG 上測試** (資料庫/模型均截止 2024 年以前)。

4. **綜合判定**：9TPG (ZIKV NS2B-NS3 + R-(+)-IRBM-Z-1) 為**無遠端別構口袋的邊界案例**；配體結合於活性/蛋白酶位點。兩個可測試的深度學習管線 (DeepAllo 官方、AlloFusion 本機) **一致給出陰性結果**，且 DeepAllo 經 1MC0 驗證管線無誤。此結果應作為「模型在缺乏別構位點結構上正確不給預測」的邊界案例報告，**不可**宣稱為 Pocket 5 的別構預測成功。
